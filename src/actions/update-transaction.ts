...
const updateTransaction = async (_currState: GenericFormState | undefined, formData: FormData): Promise<GenericFormState | undefined> => {
    ...
    const { data: { transactionId, type, amount, categoryId, date, name, notes, oldTagIdsArr, tags } } = validatedData
    ...
    try {
        ...
        console.error(error)
        return { status: "error", message: null, error: "Error updating transaction." }
    }

    return { status: 'success', message: 'Transaction successfully updated.', error: null }
}
