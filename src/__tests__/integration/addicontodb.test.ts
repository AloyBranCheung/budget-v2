...
describe('test adding icon to db util fn', () => {
    afterEach(async () => {
        const image = await prisma.images.findFirst({ where: { name: 'test-icon.png' } })
    ...
    it("should have already migrated border-all.png with no tag", async () => {
        ...
        const borderAllPng = await prisma.images.findFirst({ where: { name: filename }, include: { tag: true } })
    ...
    it("should add icon and connect to a tag", async () => {
        ...
        const tag = await prisma.tag.findFirst({ where: { name: tagName }, include: { image: true } })
