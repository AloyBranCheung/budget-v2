...
describe('test updateTransaction server action', () => {
    ...
    it('should fail auth', async () => {
        ...
        const response = await updateTransaction(defaultGenericFormState, mockErrFormData)
    ...
    it('should fail validation', async () => {
        ...
        expect(response).toBeDefined()
        expect(response!.status).toBe('error')
        expect(response!.error).toBe('transactionId: Required, oldTagIdsArr: Required, type: Required, name: Required, amount: Expected number, received nan, categoryId: Required, date: Required, notes: Required, tags: Required')
    })
    ...
    it('should change tag relationship, date, amount and name of transaction', async () => {
        ...
        formData.append('oldTagIdsArr', JSON.stringify(oldTransaction.tags.map(({ id }) => id)))
        ...
        const expectNull = await prisma.transaction.findFirst({ where: { name: oldTransaction.name } })
        expect(expectNull).toBeNull()

        const updatedTransaction = await prisma.transaction.findFirst({ where: { id: oldTransaction.id }, include: { tags: true } })
        ...
        expect(updatedTransaction?.date).toStrictEqual(new Date('2024-08-23T20:05:00.000Z'))
