...
test.describe("test user auth flow to adding paycheck to adding transaction to logging out", () => {
  test("should login, add paycheck, add transaction and logout", async ({
    page,
    loginPage,
    userCredentials,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: userCredentials.email }),
    ).toBeDefined();
    await expect(page.getByRole("heading", { name: "Welcome," })).toBeDefined();
    await expect(
      page.getByRole("heading", { name: "test@test.com" }),
    ).toBeDefined();

    const getStartedBtn = await page.getByRole("button", {
      name: "Get Started",
    });
    ...
    await expect(
      page.getByRole("heading", { name: "Add Paycheck" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Since Thursday, August 1, 2024"),
    ).toBeVisible();
